...
Project: SensingBus
Subject: Simulate the Comunication between Cloud and Fog. Simulate the compression.
'''
...
class Fog(object):
	...
	def procura_conexao_fog(self, QUEUE_TIME):
		#FileTemp = open("diffhour" + str(qntd_fog) + ".tmp", 'r')
		FileTemp = open("test_buses.tmp", 'r')
		#FileTemp = open("diffhour2.tmp", 'r')
		''' arquivo diffhour2.tmp contem os dados: id_bus, data, hora, identificacao, latitude, longitude, tamanho da msg
			Ex.: 40126 2017-05-10 03:11:06 A29083 -22.90403 -43.27335 94.0 ''''
		conexoes_fog = []
		cont = 0
		i = 0
		list_diffhour = []
		...
		for conexao in FileTemp.readlines():
			'''le cada linha do arquivo e calcula a disntacia para verificar se ha conexao entre fog e  '''
			line = conexao.split()
			...
			if ( distance <= RANGE_FOG ):
				'''armazena aos onibus conectados naquela fog '''
				conexoes_fog.append(line)

		list_diffhour = []		
		for line1 in conexoes_fog:
			''' compara cada termo da lista com todos os termos da lista para verificar se a diferenca de tempo e menor que a fila de acumulacao '''
			#print 'posicao ref' + str(line1)
			...
			for line_conexoes_fog in conexoes_fog:
				...
				'''calcula a diferenca entre os momentos de conexao com a fog '''
				diff_hour = abs(date2 - date1)
				#print 'diferenca tempo calculada ' + str(diff_hour)	
				#i = i + 1

				''' se a diferenca do tempo de conexao for menor que a fila, entao acumula o tamanho da msg'''	
				...
				'''senao, ja compacta aquele tamanho daquela unica conexao. len(conexoes_fog) == 1 serve para armazenar se a fog so tiver uma conexao '''			
				elif (int(diff_hour) != 0 or len(conexoes_fog) == 1):
					data = generateData(int(float(line1[6])))
					compression(createMessage(int(float(line1[6])), data), QUEUE_TIME)
					#print conexoes_fog[cont][6]
					#print line_conexoes_fog
							
		'''cria uma msg cujo tamanho e todos os tamanhaos de msgs acumulados por causa da fila de acumulacao'''
		if (len(list_diffhour) > 0):

			#print sum(list_diffhour)		
...
def compression(data, QUEUE_TIME):
		...
		''' This function compresses the data and it writes the data in a file.
		 ...
		 '''
		arq_compressed = open('compressed_data2_' + str(QUEUE_TIME) + '.tmp', 'a')
		compressed_data = zlib.compressobj(COMPRESSION_LEVEL, zlib.DEFLATED, WORD_SIZE_BITS, MEM_LEVEL, zlib.Z_HUFFMAN_ONLY)
		message_text = json.dumps(data)
		...
		delta_t = tf - t0
		compression_ratio = sife_after_compression / size_before_compression
		''' a definicao do ganho nao ta em dB mas eu fiz o grafico em dB e em modulo ''' 
		compression_gain = 100 * (math.log(size_before_compression / sife_after_compression))
		#print "GANHO DE COMPRESSAO: " + str(compression_gain)
...
class Simulador(Bus, Fog):
	...
	def conecta(self, qntd_fog):
		...
		if (distance <= RANGE_FOG):
			#print distance
			#time.sleep(5)	
			procurar_ultima_conexao(self.bus_node, qntd_fog)
			
				

	

def procurar_ultima_conexao(bus_node, qntd_fog):
	...
	if (len (bus_connection) > 1):
		...
		if (bus_connection[cont] != bus_node):
			...
			if (bus_connection[cont][3] == bus_connection[cont - 1][3]):
				...
				#print 'diferenca de tempo' + str(diffhour)
				#time.sleep(5)
				if (diffhour != 0):

					FileTemp = open("diffhour2" + str(qntd_fog) + ".tmp", 'a')
...
if __name__ == "__main__":
	...
	for q in Queue:
		#FileBus = open("./test_buses.tmp", 'r')
		bus_connection = []
		FileTemp = open("./sql_sbrc2018_Stops.tmp", 'r')
		#FileTemp = open("./test_fogs.tmp", 'r')

		for fog_node1 in FileTemp.readlines():

			fog_vision = Fog(fog_node1)

			#for nr_fog in NR_FOG:
				#File_nr_Fog = open("./sql_sbrc2018_Stops.tmp").read().splitlines()
				...
				#for bus_node in FileBus.readlines():
					...
					#for j in range(0, nr_fog):
						#fog_node = random.choice(File_nr_Fog)
						#fog = Fog(fog_node)
						#s = Simulador()
						#evento = s.conecta(nr_fog)
